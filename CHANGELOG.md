...
- **页脚显示形态统一**：认证页不再使用单独的紧凑页脚，全站统一由 `SiteFooter` 渲染同一响应式页脚——版权/备案与全部链接混排为单行流式布局，宽度不足时按条目自然换行，桌面端与移动端形态一致；认证页加载骨架（`AuthSkeleton`）与已登录页加载骨架（`PageSkeleton`）同步对齐新形态，避免页面间与加载前后出现页脚跳变。
