...
- README 按 Li&Design 可复用 README 模板重构：新增状态/角色/方向与技术栈徽章（shields.io 默认 flat 样式）、目录、关于信息表与项目清单，文档与设计文档入口合并；同步 Li-Design 子模块至 `f17772e`（模板仓库新增 `reusable-readme.template.md`，徽章统一默认 flat 样式）。
...
- **修复科技氛围层静止不动**：`tech-grid-pan` / `tech-beam-sweep` / `tech-dot-pulse` 三个关键帧此前只有引用、未定义，导致缓移网格、扫掠光束与呼吸光点全部不运动（光束因基态 `opacity:0` 且动画不生效而完全不可见）；补齐关键帧并把动效整体提速——网格 26s→12s、光束 8s→6s（三条错峰 0.5s/2.5s/4.5s 更频繁扫过）、光点 5.5s→3.6s 并加 7px 上下浮动、极光 16~24s→11~16s、签名描边 14s→9s、按钮扫光 5s→4s、卡片辉光 6s→4.5s（含轻微缩放）。动效约束不变：仅 `transform/opacity/background-position`，`prefers-reduced-motion` 单帧。
