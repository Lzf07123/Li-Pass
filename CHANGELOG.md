...
- 应用广场取消授权与登出联动补强：撤销授权时同步吊销该用户在此客户端上的门户会话链接（保持状态一致，并避免后续门户登出继续向已取消授权的网站派发回程通知）；撤销响应新增 `backchannel_notified` 标记。前端据此明确反馈——配置了 `logout_uri` 时经浏览器串跳退出该网站、仅配置回程通道时提示「已通知网站退出登录」、两者皆未配置时明确警告门户无法通知下线并提示用户在该网站手动退出。
