...
- **页脚显示形态统一**：认证页不再使用单独的紧凑页脚，全站统一由 `SiteFooter` 渲染同一响应式页脚——移动端上下两行居中（版权/备案行 + 链接行），桌面端左右分栏（版权/备案居左、链接居右）；认证页加载骨架（`AuthSkeleton`）与已登录页加载骨架（`PageSkeleton`）同步对齐新形态，避免页面间与加载前后出现页脚跳变。
