...
- 统一注册与登录：邮箱 + 密码，邮箱激活验证；注册完成自动跳转登录页，支持记住账号/记住密码
...
- 用户中心：资料修改、头像上传、密码修改、设备与会话管理、站内信收件箱与邮件通知开关、账号注销（手机绑定接口保留，前端暂未开放）；修改密码、2FA 开关等敏感操作支持密码二次复核（30 分钟免重复输入），注销账号必须同时输入当前密码与任意一种 2FA 验证码
- 应用广场：展示已授权网站，一键进入
