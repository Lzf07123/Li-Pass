...
| `VITE_LICENSE_NAME` / `VITE_LICENSE_URL` | 页脚与法律页面展示的开源协议名称与全文链接（构建期注入；任一置空自动隐藏入口，默认 `Apache-2.0` 与仓库 `LICENSE` 文件地址） |
...
- 入库与构建：xdb 数据（`backend/data/ip2region/`）与 Python 绑定源码（`backend/ip2region/`）均随仓库跟踪（v3.17.0，SHA256 与信任清单一致），镜像构建时直接 COPY、**不联网下载**，解决远端拉取过慢问题。更新仓库内数据/绑定时，先运行 `python scripts/download_ip2region.py --data-dir data/ip2region --binding-dir ip2region` 再提交。
- 运行期更新：管理后台「站点设置 → IP 归属地库」可手动「立即检查更新」（**后台任务 + 实时进度**：接口立即返回 202，前端每秒轮询 `GET /settings/ip2region/update/status` 展示阶段与字节级百分比，离开页面不中断），或开启自动更新（默认关闭，间隔 24 小时）。**强制 SHA256 校验**：只有列入代码内信任清单（`app/services/ip2region_pins.py`）的版本才允许安装，清单外的上游新版本一律拒绝（宁可停止更新也不装入未审计数据）；上游发布新版本后需先固定哈希更新清单并同步入库，再发布应用。更新采用下载到临时目录 → 结构校验 → SHA256 校验 → 先备份再原子替换（失败自动回滚旧库）→ 更新 meta.json，全程由跨进程文件锁互斥（多 worker 安全）；手动更新按管理员限流（默认 6 次/小时）并记审计。
- 内网部署：Gitee 镜像的 raw 端点对 xdb 数据文件返回 403（实测），不能作为运行期更新源；请保持 GitHub 默认地址。GitHub 完全不可达时只能关闭自动更新（构建已不依赖外网）。
...
邮件按操作类型区分：注册、登录二次验证、敏感操作复核、更换登录邮箱、绑定手机号各使用独立模板发送 6 位验证码（10 分钟有效，重发会作废旧码），主题与正文写明该验证码将要进行的操作；重置密码使用独立模板；邀请注册发送一次性邀请链接（7 天有效）；账号删除/登录邮箱变更发送变更通知；管理后台可发自定义通知。各发送接口的限流口径不同：

- 发信失败排查：注册/找回密码等接口返回 503「邮件发送失败」时，先看后端日志中的 `SMTPAuthenticationError`。阿里云企业邮箱（`smtp.qiye.aliyun.com:465`）返回 `526 Authentication failure` 通常是 `SMTP_PASSWORD` 填错或该邮箱开启了「三方客户端安全密码」——请填写邮箱登录密码或管理后台生成的客户端安全密码，且 `SMTP_USERNAME` 必须是完整邮箱地址、不要带空格；发信还需该账号在企业邮箱后台开启 SMTP 服务。
...
- **默认方案**：注册验证邮箱（普通注册验证、邀请注册、管理员代建）后自动启用「邮箱验证码」；账号必须至少保留一种 2FA 方案，关闭最后一种会被拒绝。TOTP 认证器为可选升级方案。
- **历史数据迁移**：升级时迁移 `a2b3c4d5e6f7` 会把所有「已验证邮箱且无任何 2FA」的用户批量启用邮箱验证码；其 downgrade 为 no-op（无法恢复用户此前手动关闭的历史意图）。即使迁移遗漏，登录兜底也会自动开启并进入 2FA。
- **管理端重置 2FA**：`POST /api/v1/admin/users/{id}/reset-2fa` 恢复默认邮箱验证码、清空 TOTP 与恢复码并吊销该用户全部会话（不再把账号清成 1FA）。
- **注销/删除账号双因素复核**：注销账号、删除用户、批量删除必须同时提供当前密码与任意一种 2FA 验证码（邮箱验证码或 TOTP），且不受 30 分钟免复核窗口豁免；`POST /api/v1/me/step-up/send` 发送复核邮箱验证码，与登录 2FA 共用发送冷却（60 秒）与每小时配额。
- **邮箱不可达时的兜底**：用户可提前开启 TOTP 并使用恢复码登录；若邮箱与 TOTP 均不可用，需由管理员协助（重置 2FA 后仍需邮箱验证码，因此请先确认账号邮箱可用）。
- **未验证邮箱**：账号验证邮箱前仍可密码登录（无法接收验证码）；完成邮箱验证后立即强制 2FA。
