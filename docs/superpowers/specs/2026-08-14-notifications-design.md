...
| `recalled_at` | timestamptz nullable | 站内信撤回时间（撤回后收件箱不再展示） |
...
| `POST` | `/api/v1/admin/notifications/{id}/recall` | 撤回站内信（仅 `in_site` 通知；邮件已发出无法撤回）。幂等，返回 `{recalled}` |
...
1. 管理员在发送历史中撤回某条站内信，接口校验通知存在且为站内信渠道。
2. 写入 `notifications.recalled_at`（幂等）并记审计 `admin_recall_notification`。
3. 用户收件箱列表与未读数查询一律过滤 `recalled_at` 非空的收件行：消息从收件箱消失，已读未读均不再展示；已发出的邮件不受影响。

## 6. 前端

- 管理端：`/admin/notifications` 新增「通知管理」标签页，上半部撰写表单（收件人单选 + 已注册用户勾选列表（支持搜索与已选计数）、渠道复选、标题、正文、占位符说明），下半部发送历史表格；站内信可撤回（确认弹窗），已撤回记录显示「已撤回」状态。
...
- 后端（撤回）：撤回站内信后收件箱为空、重复撤回幂等、邮件渠道不可撤回、非管理员 403、审计落库与历史 `recalled_at`。
