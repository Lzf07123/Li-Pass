...
- 用户管理：搜索（邮箱/昵称）、查看、禁用/启用、手动重置密码、重置用户 2FA、管理员代建账号、邀请注册/批量邀请、批量启用/禁用/删除；角色变更、重置密码、重置 2FA、批量删除与删除账号等敏感操作均需管理员本人当前密码复核。
...
- 登录按 IP 前置限流 + 邮箱/IP 计数（默认 5 次/15 分钟，第 6 次失败 429）；2FA 挑战错误上限 5 次，超限作废；不做账号级冻结。
